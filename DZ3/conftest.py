...
@pytest.fixture()
def make_folders():
    return checkout(f"mkdir {data['folderin']} {data['folderout']}", "")


@pytest.fixture()
def clear_folders():
    return checkout(f"rm -rf {data['folderin']}/* {data['folderout']}/*", "")
...
def make_subfolder():
    ...
    if not checkout(f"cd {data['folderin']}; mkdir {subfoldername}", ""):
        return None, None
    if not checkout(f"cd {data['folderout']}/{subfoldername}; dd if=/dev/urandom of={testfilename} bs=1M count=1 iflag=fullblock", ""):
